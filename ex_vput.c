...
vinschar(int c)
{
	...
	if ((!IM || !EI) && ((hold & HOLDQIK) || !value(REDRAW) || value(SLOWOPEN))) {
		...
		vputchar(c);
		if (DEPTH(vcline) * WCOLS + !value(REDRAW) > destcol)
			return;
		/*
...
vishft()
{
	...
	if (!IM && !EI) {
		...
		if (slakused > tabslack && inscol + doomed != tabstart) {
			i -= slakused - tabslack;
			slakused -= tabslack;
		}
	...
	char c;
{
	...
	if (!IM || !EI) {
		...
		if (!IN) {
			 ...
			 */
			tp = vtube0;
			for (j = (inscol + inssiz - 1) / WCOLS + 1; j <= (tabstart + inssiz - doomed - 1) / WCOLS; j++) {
				vgotoCL(j * WCOLS);
				i = inssiz - doomed;
				up = tp + j * WCOLS - i;
				goim();
